...
(d/q '[:find ?e ?name
     ...
     mages)
;; => #{[:m/ivan "Ivan"] [:m/david "David"] [:m/john "John"]}
...
(d/q '[:find ?age .
       :where
       [?mage-id :mage/name "John"]
       [?mage-id :mage/age ?age]]
     mages)
...
(defn- centuries [x] (int (/ x 100)))

(d/q '[:find ?name ?centuries
       :where
       [?e :mage/name ?name]
       [?e :mage/age ?age]
       ;; делим возраст на сто и связываем результат с промежуточной переменной `?hths`
       [(otus-24.core/centuries ?age) ?centuries]] ; округляем вниз и связываем с `?centuries`
     mages)
...
;; факт о том, что какие-то данные были удалены
...
(def john (d/entity (d/db conn) [:mage/name "John"]))
;; => #object[datascript.impl.entity.Entity "0x7d82c310" "#:db{:id 2}"]
...
;;     :mage/age 75,
;;     :mage/name "John",
;;     :mage/spells [{:db/id 5, :spell/manacost 100, :spell/name "Frostbolt"}]}

;; Reverse lookups
(d/pull (d/db conn) '[{:mage/_spells [:mage/name]}] [:spell/name "Frostbolt"])
;; => {:mage/_spells [{:mage/name "John"}]}
...
(d/pull (d/db conn) '[:mage/name :mage/age {:mage/apprentices 1}] [:mage/name "Ivan"])
;; => {:mage/age 200,
;;     :mage/apprentices [{:mage/age 75, :mage/name "John"}],
;;     :mage/name "Ivan"}
